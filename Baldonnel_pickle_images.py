...
categories_list =  [{"supercategory": "outdoor", "id": 1, "name": "wheelie bin"},{"supercategory": "accessory", "id": 2, "name": "suitcase"},{"supercategory": "accessory", "id": 3, "name": "backpack"},{"supercategory": "outdoor", "id": 4, "name": "radiation barrel"},{"supercategory": "outdoor", "id": 5, "name": "gas cylinder"},{"supercategory": "outdoor", "id": 6, "name": "cable hose"},{"supercategory": "outdoor", "id": 7, "name": "car battery"}]
...
#initializing a 4d array consisting of 3d arrays of shape(1024,1024,3)
array_of_images_4d =[np.zeros((576, 1024, 3), dtype = np.uint8) for i in range(0,1261)]
array_of_images_4d = np.array(array_of_images_4d)
#Loading all the train images iteratively and packing them onto a 4d array
counter = 1
for id in range(len(list_of_all_image_ids_train)):
    ...
    if counter%50 == 0:
        print(counter)
    # print(id)
    current_image = dataset_train.load_image(id)
    image_id = dataset_train.image_info[id]["id"]
    array_of_images_4d[image_id,:,:,:] = current_image
    #image_size_list.append(sys.getsizeof(current_image))
    counter = counter + 1 
...
for id in range(len(list_of_all_image_ids_val)):
    ...
    if counter%10 == 0:
        print(counter)
    current_image = dataset_val.load_image(id)
...
start = timer()
with open('/ichec/work/ngcom012c/maskrcnn/numpy_images_and_masks/numpy_array_of_images.pickle_v2', 'wb') as f:
    # Pickle the 'data' dictionary using the highest protocol available.
    pickle.dump(array_of_images_4d, f, pickle.HIGHEST_PROTOCOL)
#reading from array 
end = timer()
print("Time taken for pickling"+str(total_number_of_images_train_plus_val)+"from RAM(stored in a 4d array): "+str(end - start))

np.equal(array_of_images_4d[dataset_val.image_info[4]["id"],:,:,:],dataset_val.load_image(4)))
# start = timer()



# start = timer()
# for id in range(total_number_of_images_train_plus_val):
    # temp_image = array_of_images_4d[id,:,:,:]
# end = timer()
# print("Time taken for loading"+str(total_number_of_images_train_plus_val)+"from RAM(stored in a 4d array): "+str(end - start))
