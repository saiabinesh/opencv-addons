...
# found = client.simSetSegmentationObjectID("FbxScene_PickupTruck[\w]*", 3, True);
# print("found FbxScene_PickupTruck? ",found)
# found = client.simSetSegmentationObjectID("KamazNov2[\w]*", 3, True);
# print("found KamazNov2? ",found)
# found = client.simSetSegmentationObjectID("FbxScene_JCB2[\w]*", 3, True);
# print("found FbxScene_JCB2? ",found)
# found = client.simSetSegmentationObjectID("Cube_2[\w]*", 3, True); #Floor mat of the pickup truck which is of a different ID
...
#Setting unique IDs for gas cylinders
# found = client.simSetSegmentationObjectID("Propane_Tank_E_Blueprint[\w]*", 6, True);
...
def draw_bounding_box(filename,image_count):
	...
	if len(contours)>1:
		...
		min_x, min_y, max_x, max_y = 2050,2050,0,0
		...
		bounding_box_coordinates.append("object: train_"+str(object_count)+", x: "+str(centre_x)+", y: "+str(centre_y)+", width: "+str(w)+", height: "+str(h))
		if(len(bounding_box_coordinates)!=0):
			print(bounding_box_coordinates)
			with open("D:/AirSim/New/Images/Images_master/train_bbox_coordinates_"+str(image_count)+".txt", "w+") as file:
	...
	for x in np.linspace(2.93,-37.37, 10):
		for y in np.linspace(-88.17, -192.27,10):
			...
			for idx, response in enumerate(responses):
				global filename
				filename = 'D:/AirSim/New/Images/Images_master/segmented_train_'+str(i)
