...
def create_object_masks(object_IDs_list, class_label, image_directory):
    #################################################################### creating masks only for gas cylinder 3########################################
    #Set 0 for any other object. 
    object_count = 3
    for object_ID in object_IDs_list:
        ...
        ###################################### uncomment for any other object #################################
        ...
        ################## Overriding the general object_ID function as train has multiple mesh IDs for the same single object
        #Set train alone to 2. Means setting the cars as well
...
#Arbitrary function that can take in a folder as input and delete all empty images inside it.            
...
def test_check_object_present():
    ...
    for truck_number in truck_numbers:
        ...
        # found = client.simSetSegmentationObjectID("[\w]*", 0, True);
        # print("Set everything to 0 color code: %r" % (found))
        # found = client.simSetSegmentationObjectID("Propane_Tank_E_Blueprint"+str(truck_number), 2, True);
        # print("Setting color for Propane_Tank_E_Blueprint"+str(truck_number),found)
        i =1 #resetting i, the image count, before moving on to the next instance of the object
        object_exists_list = [] #initializing a list that can hold whether a truck exists in each of these images
        ...
        for z in np.linspace(-30,-100,10):
            for x in np.linspace(2.93,-37.37, 10):
                for y in np.linspace(-88.17, -192.27,10):
                    global filename
                    filename = 'D:/AirSim/New/Images/Images_master/image_'+str(i)+'_gas truck_'+str(object_count)
                    object_exists_list = check_object_present(filename, i,object_exists_list)
                    if i%50 == 0:
                        print("Image count = ", i)
                    i = i +1
        text_filename = "list_gas_truck_"+str(object_count)+".txt"
        buffsize=1
        file = open(text_filename,"a+",buffsize)
        print("Object count: ",object_count )
        print(object_exists_list)
...
gas_object_list = ["Propane_Tank_E_Blueprint10_2"]
...
create_object_masks(gas_object_list, current_class_label, current_image_directory)
