	...
	if len(contours)>1:
		for contour in contours[:-1]: #ignoring the last contour because it's the one around the whole picture
			...
			if w*h>2000:
				...
				bounding_box_coordinates.append("object: truck_"+str(object_count)+", x: "+str(centre_x)+", y: "+str(centre_y)+", width: "+str(w)+", height: "+str(h))
		if(len(bounding_box_coordinates)!=0):
			print(bounding_box_coordinates)
			with open("D:/AirSim/New/Images/Images_master/truck_bbox_coordinates_"+str(image_count)+".txt", "w+") as file:
	...
	for x in np.linspace(2.93,-37.37, 10):
		for y in np.linspace(-88.17, -192.27,10):
			...
			for idx, response in enumerate(responses):
				global filename
				filename = 'D:/AirSim/New/Images/Images_master/segmented_truck_'+str(i)
