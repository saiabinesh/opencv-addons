import datetime
import json 
global date = str(datetime.datetime.now())
print(date)
...
def create_annotations(category_id_list, folder_path, master_json_dict, path_binary_list_of_flags):
	...
	for image_number in range(1,10):
		...
		"width": 2000,
		"height": 2000,
		"file_name": folder_path+"/raw_"+str(image_number)+".jpg",
		"license": 1,
		"flickr_url": "none",
		"coco_url": "none",
		"date_captured": date,
	...
	for i in range(0,len(category_id_list)):
		...
		for instance_number in (1,len(number_of_instances_in_category)+1):
			...
			binary_flags_object_present = (path_binary_list_of_flags+"/list_"+current_category_name+"_"+str(instance_number)+".txt").read().splitlines())
