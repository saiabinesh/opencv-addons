...
def check_object_present(filename,image_count,object_exists_list,img_rgba):
	...
	if len(contours)>1:
		...
		if len(contours)>2:
			print("contour length ="+str(len(contours))+" image_count ="+str(image_count))
		print("unique values in numpy array of image = ",np.unique(img_rgba))
	else:
	...
	for object_ID in object_IDs_list:
		...
		if object_count>5:
			...
			if found:
				...
				for z in [-10]:
					for x in np.linspace(97.85, -26.81,21):
						for y in np.linspace(-200.60,-109.40,10):
									...
									if i>=883:
										...
										for idx, response in enumerate(responses):
											...
											if response.pixels_as_float:
												...
												# print("unique values in numpy array of image = ",len(np.unique(img_rgba)))
												if np.sum(img_rgba)!=323223552:
													print(np.sum(img_rgba))
													print("i=",i)
													airsim.write_png(os.path.normpath(filename + 'lowq.png'), img_rgba) #write to jpg
													print(filename)
													...
													# for idx, response in enumerate(responses):
														...
														# img_rgba = img1d.reshape(response.height, response.width, 4) #reshape array to 4 channel image array H X W X 4
														# img_rgba = np.flipud(img_rgba) #original image is flipped vertically
														# airsim.write_png(os.path.normpath(filename + '_raw.png'), img_rgba)
													print("--- %s minutes ---" % ((time.time() - start_time)/60))
													# exit()
										#object_exists_list = check_object_present(filename, i,object_exists_list,img_rgba)
										# if (i==900):
											# exit()
										# if i%100 == 0:
											# # print("Image count = ", i)
				...
				buffsize=1
				file = open(text_filename,"w",buffsize)
...
current_image_directory = "C:/AirSim/New/Images/Images_master_v6"
