	...
	for image_number in list_of_image_numbers:
		...
		for i in range(0,len(category_id_list)):
			...
			for instance_number in range(1,number_of_instances_in_category+1):
				...
				#if not ((len(binary_flags_object_present)) == 500):
					# print(len(binary_flags_object_present))
					# print((path_binary_list_of_flags+"/list_"+current_category_name+"_"+str(instance_number)+".txt"))
...
seed = 30
random.seed(seed)
validation_set = random.sample(range(401,501),10)
# training_set = [i for i in range(1,401) if i not in validation_set]
training_set = [i for i in range(401,501) if i not in validation_set]
# print(training_set)
